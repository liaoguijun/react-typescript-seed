 ...
 *    ex: [] => ... => [ ['foo', 'bar1', 'bar2', 'bar3'], ['foo.bar1', 'bar2', 'bar3']]
 */
...
const deepFindDirty = (obj, path) => {
  ...
  const setAllPaths = new Set(allPaths.map((p) => p.join(HASHTAG)));
  // Delete duplicates
  setAllPaths.forEach((v1, v2, values) => allPathsWithoutDuplicate.push(v2.split(HASHTAG)));
  // Test all possibles paths while traduction is not found
  for (let j = 0; j < allPathsWithoutDuplicate.length; j += 1) {
    if (trad === undefined) {
      // Test a path
      for (let i = 0; i < allPaths[j].length; i += 1) {
...
const showMissingOrDefault = (key, children) => {
  const { renderInnerTextForMissingKeys } = TranslatorContext.context;
...
export default class Translate extends React.Component<ITranslateProps> {
  ...
  shouldComponentUpdate() {
    const currentLocale = TranslatorContext.context.locale
  ...
  render() {
    const {
      contentKey, interpolate, component, children,
    } = this.props;
